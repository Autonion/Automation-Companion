        ...
        wakeAndDo(context) {
            ...
            if (acc != null) {
                acc.attemptToggleLocation { success ->
                    ...
                    // Start tracking service for slot regardless; service will verify Location status
                    if (slotId != -1L) {
                ...
                // Accessibility not enabled — just start service and show fallback notification
                if (slotId != -1L) {
                    TrackingForegroundService.startForSlot(context, slotId)
                } else {
                    TrackingForegroundService.start(context)
...
//    }
    ...
    private fun handleStop(context: Context, slotId: Long) {
        wakeAndDo(context) {
            // Attempt to toggle off via Accessibility, then stop service
            val acc = TileTogglerAccessibilityService.instance
            if (acc != null) {
                acc.attemptToggleLocation { success ->
                ...
                // No accessibility service; just stop the service
                if (slotId != -1L) {
                    TrackingForegroundService.stopForSlot(context, slotId)
                } else {
                    TrackingForegroundService.stop(context)
